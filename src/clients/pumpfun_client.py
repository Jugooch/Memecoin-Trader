...
class PumpFunClient:
    ...
    async def create_buy_transaction(self, wallet_pubkey: str, mint_address: str, sol_amount: float, slippage_bps: int = 1500) -> Dict:
    ...
    async def create_sell_transaction(self, wallet_pubkey: str, mint_address: str, token_amount: float, slippage_bps: int = 300) -> Dict:
        ...
        try:
            ...
            response = await self._make_pump_portal_local_request({
                ...
                "action": "sell",
                "mint": mint_address,
                "amount": token_amount,  # Raw token count, not smallest units!
