...
class PumpFunClient:
    ...
    async def create_buy_transaction(self, wallet_pubkey: str, mint_address: str, sol_amount: float, slippage_bps: int = 3000) -> Dict:
        ...
        try:
            ...
            response = await self._make_pump_portal_local_request({
                ...
                "priorityFee": 0.003,  # Higher priority fee for faster execution (~$0.75)
    ...
    async def create_sell_transaction(self, wallet_pubkey: str, mint_address: str, token_amount: float, slippage_bps: int = 2000) -> Dict:
        ...
        try:
            ...
            # NOT raw units (e.g., 1000500000 for 6 decimals)
            # When denominatedInSol=false, Pump Portal expects UI amount
            response = await self._make_pump_portal_local_request({
                "publicKey": wallet_pubkey,
                "action": "sell",
                "mint": mint_address,
                "amount": token_amount,  # UI amount (e.g., 1000.5 tokens, not 1000500000 raw units)
                "denominatedInSol": "false",  # Amount is in tokens (UI amount)
                "slippage": int(slippage_bps / 100),  # Send as number, not string!
                "priorityFee": 0.003,  # Higher priority fee for faster execution (~$0.75)
                "pool": "auto"  # Let it decide - now that we have proper price calculations
            })
